...
macOS와 WSL2 로컬 실행에는 다음 도구가 필요하다. 일반 Linux와 Windows native는
아직 지원 범위가 아니다.
...
- WSL2: `setsid`, `nohup`, `mktemp`, `tr`, `/proc`
...
tmp_dir="$(mktemp -d)" && rmdir "$tmp_dir"
printf '%s\n' ok | tr '[:lower:]' '[:upper:]'
grep -Eqi 'microsoft.*wsl2' /proc/sys/kernel/osrelease
```
...
WSL2에서는 배포판의 패키지 관리자로 Go, Flutter, make, curl, `setsid`를 제공하는
`util-linux`, 그리고 `nohup`·`mktemp`·`tr`을 제공하는 coreutils를 설치한 뒤 Flutter
web을 활성화한다. systemd 설정은 필요하지 않다.
...
- Liquid2 API가 `HTTP ok`, service `loaded`(macOS) 또는 `running`(WSL2)
...
WSL2의 서비스 상태와 stdout/stderr 로그는
`${XDG_STATE_HOME:-$HOME/.local/state}/<product>/services/` 아래에 보관된다. 직접
경로를 추측하지 말고 `logs` 명령으로 확인한다.

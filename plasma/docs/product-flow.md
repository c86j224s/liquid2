...
`report`는 현재 구현과 장부에서 쓰는 내부 artifact 이름이지만, 사용자 가치의
중심은 원자료의 주석이나 조사 로그가 아니다. Plasma의 산출물은 사용자가 새 정보를
이해하고, 여러 자료를 연결해 인사이트를 얻고, 시장조사나 의사결정에 쓸 수 있으며,
긴 글이어도 계속 읽고 싶게 만든 가공본이어야 한다.

따라서 리포트 작성은 소스 목록을 친절하게 주석 처리하는 일이 아니다. 에이전트는
원자료를 소화한 뒤 독자가 붙잡을 질문, 의외성, 긴장, 연결되는 통찰, 다음 문단을
읽을 이유를 글의 흐름으로 만든다. 근거 보존, 한계 표시, 출처 추적성은 필수 품질
조건이지만, 사용자에게 보이는 표면을 지배해서는 안 된다. 산출물이 길어도 괜찮지만
각 부분은 독자의 이해나 호기심을 앞으로 이동시켜야 한다.

현재 기본 경로는 F4 작성 지침과 시각자료 계획을 유지하면서, 계획에 중심 질문,
독자가 가져갈 결론, 읽는 순서, 반드시 남길 정보, 요약하거나 보조 층으로 옮길
정보를 담는 얇은 작성 계약을 추가한다. 새 장문 리포트의 기본 경로는 여기에 독자의
호기심과 이해가 이어지는 순서, Section의 직접 설명, 필요한 읽기 역할이 없는 Part
도입·전환·마무리의 생략을 더한다. 작성자는 원천을 소화한 뒤 원천을 따로 읽지 않을
독자에게 주제를 직접 설명한다. 이전 대화, 조사 답변, controller 질문은 작업
기억으로만 쓰고 source로 인용하지 않는다. 에이전트는 원자료 기반 사실, 해석과
함의, 약한 신호, 충돌과 열린 질문을 독자에게 자연스러운 구조로 정리한 뒤 풍부한

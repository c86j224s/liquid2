...
7. 사용자가 제한된 자율 진행을 시작한다.
   ...
   사용자가 "이 미션을 조금 더 진행해줘"처럼 명시적으로 요청하면 Plasma는
   `workflow.run.requested`를 미션 장부에 남긴다. 실행은 작은 step 수와 유한한
   시간으로 제한된다. 기본 예산은 최대 10 step과 최대 25분이다. 각 step은 일반
   ...
   `plasma.workflow.start/status/stop` 도구로 요청과 중지만 기록한다. MCP 도구
...
8. workflow run이 중지되거나 완료된다.
   ...
   `workflow.run.stopped`로 닫힌다. 사용자 중지 없이도 최대 step 수, 최대 실행 시간,
...
10. 사용자가 리포트를 만든다.
   ...
   workflow run이 단계 제한이나 시간 제한에 닿았지만 에이전트가 다음 조사가 필요하다고

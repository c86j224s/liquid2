...
| H5 | 수동으로 실행하던 post-canonical 한국어 말투 보정 단계입니다. 브라우저에서는 deprecated이며 historical/API compatibility로만 남습니다. 새 브라우저 장문 요청이 항상 enabled로 보내는 pre-canonical `post_report_humanize`와는 별도 경로입니다. | planning이나 source selection에 참여하지 않으며 새 브라우저 UI에는 노출하지 않습니다. 장문이 아닌 브라우저 요청은 `post_report_humanize`를 disabled로 유지합니다. |

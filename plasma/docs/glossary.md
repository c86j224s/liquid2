...
| Report | A Plasma-owned output artifact assembled from mission work. The internal name is report, but the user value is not source annotation or an investigation log; it is a processed reading artifact that helps the user understand new information, connect sources, and keep reading. Markdown is the primary report artifact; HTML is a rendering/export. |
| Final assembly | Server-owned deterministic assembly of reviewed long-form Parts into one immutable Markdown artifact for staged final editing. | No provider session or MCP tool is involved; v2 records `report.final_assembly.created` before final writer work. |
| Final writer | The first provider-backed v2 final-edit stage after deterministic final assembly. | Works only through `plasma.report.long_form.final_write.*` and may improve opening, conclusion, Part transitions, and whole-report connective logic without adding research or reordering complete Parts/Sections. |
...
| H5 | Korean humanization pass. Manual or legacy exports run it after the canonical Markdown report through bounded MCP patch tools; staged long-form final-edit pipelines reuse the existing `post_report_humanize` setting only as an optional pre-canonical style edit. | Optional/secondary tone work; not part of planning or source selection, and not a new user-facing long-form option. |
...
| C4 | Long-form report assembly strategy that preserves section bodies and performs limited heading normalization instead of rewriting the whole report. | Productized for stored legacy assembly and replay semantics; new narrative-contract long-form reports preserve reviewed Part outputs before v2 final assembly, final writer, reader, optional style, and corrective gate. Stored v1 plans keep reader/style/gate replay behavior. |

...
| H5 | Korean humanization pass. Manual/post-canonical exports are deprecated and remain only for historical/API compatibility; staged long-form final-edit pipelines use `post_report_humanize` as an optional pre-canonical style edit. | Optional/secondary tone work; not part of planning or source selection. The browser exposes it only as an opt-in long-form setting. |

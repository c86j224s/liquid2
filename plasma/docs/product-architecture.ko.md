...
미션 목록은 전역 notification 장부를 만들지 않고, 각 미션 장부에서 `last_sequence`, 진행 중 작업,
분류된 최신 종료 작업만 더한 activity summary를 읽습니다. SQLite는 목록에 필요한 event subset을 한 번에
읽고, browser는 미션별 확인 sequence만 localStorage에 둡니다. 목록에서 사라진 미션의 local read state는
제거하며, 이미 관찰한 미션에 진행 중 작업이 있을 때만 그 미션의 summary를 주기적으로 갱신합니다. 미션 상세를 성공적으로
열면 그 시점의 local watermark만 전진하고 server ledger나 product state는 바꾸지 않습니다.
...
새 run은 기본 20 step이며 전체 workflow 실행 시간 제한은 없습니다. 기본값 적용 뒤
`max_steps`는 1..20, `max_duration_ms`는 0..86400000 범위만 허용합니다. duration 0은
run 전체 경과 시간 예산이 없음을 뜻하고, 양의 `max_duration_ms`만 기존 예산을 적용합니다. 각 workflow step은
...
Plasma가 spawn한 기본 Codex/Claude 연구 에이전트의 MCP 도구 목록에는
`plasma.workflow.status`와 `plasma.workflow.stop`만 포함하며 `plasma.workflow.start`는
포함하지 않습니다. 브라우저와 CLI의 workflow start는 그대로 사용할 수 있고, 명시적으로
...
Web mission detail에는 선택한 mission만 대상으로 하는 `active_work` 읽기 모델을
추가합니다. 이 모델은 durable state에서 모든 열린 agent turn, report pending event,
terminal이 아닌 workflow run을 투영하고, 안정적인 사유 코드, 영향받는 control, 정확한
취소 또는 보기 행동을 함께 제공합니다. Browser는 mission 전환 때 요청 로컬 busy/notice 상태를
비운 뒤 이 투영을 다시 렌더링합니다. report in-flight handle은 process-local 취소와
ownership 보조 수단으로만 남으며 product state가 아닙니다.

...
힌트는 소스나 미션 설정이 아닙니다. Plasma는 이후 보고서 요청으로 힌트를 복사하지 않으며, 대화·말투 보정·보고서 수정·HTML 내보내기 프롬프트에도 힌트를 다시 넣지 않습니다. 자동 계획형·장문 보고서의 최초 계획은 새 제공자 세션에서 시작하고 이후 작성은 그 계획 세션을 이어가므로, 앞선 보고서 프롬프트를 상속하지 않습니다. 명시적으로 같은 세션을 이어 쓰는 호환 경로에서는 기록이 남아 있을 수 있습니다.

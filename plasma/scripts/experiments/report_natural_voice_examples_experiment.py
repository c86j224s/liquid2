...
from report_natural_voice_examples.prompts import freeze_protocol, lint_example_prompt
from report_natural_voice_examples.review import make_semantic_audit_pack, record_semantic_audit
from report_natural_voice_examples.runner import authorize_full, run_calibration, run_full, run_pilots
from report_natural_voice_examples.summary import export_summary


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run sealed natural-voice example experiment 58")
    parser.add_argument("action", choices=(
        "prepare", "lint-prompt", "calibrate", "freeze", "run-pilots", "authorize-full",
        "run-full", "make-blind-packets", "lock-verdicts", "make-audit-pack", "lock-audit",
        "summarize", "verify",
    ))
...
def main() -> None:
    ...
    if args.action == "prepare":
        ...
        result = lint_example_prompt(archive, _required(args.prompt, "--prompt"))

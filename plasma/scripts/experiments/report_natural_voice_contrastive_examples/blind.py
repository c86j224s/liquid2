...
def make_blind_packets(
    ...
    for index, file_id in enumerate(file_ids, 1):
        ...
        contrastive_slot = rng.choice(("A", "B"))
        control_slot = "B" if contrastive_slot == "A" else "A"
        bodies = {
            control_slot: _candidate(archive, records[(file_id, "control")]),
            contrastive_slot: _candidate(archive, records[(file_id, "contrastive")]),
        }
        ...
        mappings.append({
            ...
            "contrastive_slot": contrastive_slot,
...
def validate_private_mapping(archive: ExperimentArchive) -> dict[str, object]:
    ...
    for row in rows:
        if not isinstance(row, dict) or set(row) != {
            "packet_id", "file_id", "control_slot", "contrastive_slot", "packet_path", "packet_sha256"
        }:
            raise BlindError("private mapping row schema mismatch")
        if {row["control_slot"], row["contrastive_slot"]} != {"A", "B"}:

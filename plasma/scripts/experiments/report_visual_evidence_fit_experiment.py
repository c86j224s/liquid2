#!/usr/bin/env python3
"""Issue #174 visual evidence-fit experiment runner."""
...
EXPERIMENT_ID = "27-report-visual-evidence-fit-2026-07-22"
ARMS = ("visual_plan", "visual_evidence_fit")
PROFILE_BY_ARM = {
    "visual_plan": "visual-plan",
    "visual_evidence_fit": "visual-evidence-fit",
}
...
def analyze(args: argparse.Namespace) -> None:
    ...
    for (topic, mode), arms in sorted(by_key.items()):
        if all(arm in arms and arms[arm]["status"] == "completed" for arm in ARMS):
            ...
            candidate = arms["visual_evidence_fit"]
            pairs.append(
                {
                    "topic": topic,
                    "mode": mode,
    ...
    for pair in analysis["pairs"]:
        ...
        packet = {
            ...
            "review_questions": [
                ...
                "Does each visual's precision match the evidence level instead of implying unsupported certainty?",

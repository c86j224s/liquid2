...
func TestCompleteReportRunActualRecordedExactOutcome(t *testing.T) {
	...
	if usageEvent.EventType != ReportAgentUsageRecordedEventType || usageEvent.Producer.ID != "ses_1" || usageEvent.CausationEventID != "evt_target" || usageEvent.CorrelationID != "evt_root" {
...
func TestCompleteReportRunRejectsActualInvalidOutcomeAndRollsBack(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			s := completionTestStore{events: baseCompletionEvents()}
			_, err := CompleteReportRun(context.Background(), &s, ReportCompletionRequest{MissionID: "mis_1", CanonicalEventID: "evt_art", ActualUsage: &ReportAgentUsageRequest{MissionID: "mis_1", CanonicalEventID: "evt_target", AgentSessionID: "ses_1", PreviousAgentSessionID: "ses_1", Surface: "report_requirements", Usage: tc.usage}})

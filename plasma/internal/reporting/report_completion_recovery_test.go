...
func TestRecoverMissionSyntheticOutcomeHasExactEnvelopeAndReplay(t *testing.T) {
	...
	if usage.EventType != ReportAgentUsageRecordedEventType || usage.Producer != (ledger.Producer{Type: "agent_session", ID: "ses"}) || usage.CausationEventID != "evt_target" || usage.CorrelationID != "evt_root" {

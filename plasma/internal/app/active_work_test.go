...
func TestActiveWorkFromMissionStateUsesOnlyCurrentMissionDurableState(t *testing.T) {
	events := []ledger.Event{testActiveWorkEvent(t, "evt_turn", "turn.agent.pending", map[string]any{"user_event_id": "evt_user"})}
	state := ActiveWorkFromMissionState(events, nil)
	if len(state.Items) != 1 || state.Items[0].ReasonCode != mission.BlockingReasonAgentTurn || state.Items[0].Action != "cancel_turn" {
	...
	state = ActiveWorkFromMissionState(nil, []workflowstate.WorkflowRunView{{WorkflowRunID: "wfr_1", Status: WorkflowStatusRunning}})
	if len(state.Items) != 1 || state.Items[0].ReasonCode != mission.BlockingReasonWorkflow || state.Items[0].WorkflowRunID != "wfr_1" {
...
func TestActiveWorkMatrixPreservesTurnAndQueuedWorkflow(t *testing.T) {
	events := []ledger.Event{testActiveWorkEvent(t, "evt_turn", "turn.agent.pending", map[string]any{"user_event_id": "evt_user"})}
	state := ActiveWorkFromMissionState(events, []workflowstate.WorkflowRunView{{WorkflowRunID: "wfr_queued", Status: WorkflowStatusQueued}})
...
func TestActiveWorkExcludesTerminalLedgerAndWorkflowState(t *testing.T) {
	...
	state := ActiveWorkFromMissionState(events, []workflowstate.WorkflowRunView{{WorkflowRunID: "wfr_1", Status: WorkflowStatusCompleted}})
	if len(state.Items) != 0 || len(state.Blocks) != 0 {
		t.Fatalf("terminal work must not remain active: %#v", state)
	}
}

func testActiveWorkEvent(t *testing.T, id, eventType string, payload any) ledger.Event {
	t.Helper()
	raw, err := json.Marshal(payload)
	if err != nil {
		t.Fatal(err)
	}
	return ledger.Event{EventID: id, EventType: eventType, Payload: raw}
}

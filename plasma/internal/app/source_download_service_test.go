...
func (s *sourceDownloadTestStore) ListLedgerEvents(context.Context, string) ([]LedgerEvent, error) {
	return s.events, nil
}
func (s *sourceDownloadTestStore) GetRawArtifact(_ context.Context, id string) (RawArtifact, error) {
	a, ok := s.artifacts[id]
	if !ok {
		return RawArtifact{}, sql.ErrNoRows
	}
	return a, nil
}
func (s *sourceDownloadTestStore) GetSourceSnapshot(_ context.Context, id string) (SourceSnapshot, error) {
	if s.snapshot.SnapshotID == "" || s.snapshot.SnapshotID != id {
		return SourceSnapshot{}, sql.ErrNoRows
	}
	return s.snapshot, nil
}
func (s *sourceDownloadTestStore) ListSourceSnapshots(context.Context, string) ([]SourceSnapshot, error) {
...
func TestResolveSourceCandidateDownloadLifecycleAndConsumption(t *testing.T) {
	...
	store := &sourceDownloadTestStore{events: []LedgerEvent{staged}, artifacts: map[string]RawArtifact{
	...
	cases := []struct {
		...
		{"cross mission artifact", func(s *sourceDownloadTestStore) {
			s.artifacts["art_candidate"] = RawArtifact{ArtifactID: "art_candidate", MissionID: "mis_2", Content: []byte("x")}
		}},
		...
		{"persisted same artifact consumed", func(s *sourceDownloadTestStore) {
			s.snapshots = []SourceSnapshot{{SnapshotID: "src_1", MissionID: "mis_1", ArtifactIDs: []string{"art_candidate"}}}
		}},
		{"persisted same URL different proposal consumed", func(s *sourceDownloadTestStore) {
			s.snapshots = []SourceSnapshot{{SnapshotID: "src_1", MissionID: "mis_1", Connector: ConnectorRef{ExternalURI: "https://example.com/doc"}}}
		}},
		{"persisted Confluence locator consumed", func(s *sourceDownloadTestStore) {
			...
			s.snapshots = []SourceSnapshot{{SnapshotID: "src_1", MissionID: "mis_1", Locators: json.RawMessage(`[{"site_url":"https://example.atlassian.net/wiki","page_id":"123"}]`)}}
		}},
		...
		{"unrelated persisted snapshot remains downloadable", func(s *sourceDownloadTestStore) {
			s.snapshots = []SourceSnapshot{{SnapshotID: "src_other", MissionID: "mis_1", Connector: ConnectorRef{ExternalURI: "https://example.com/other"}}}
		}},
	}
...
func TestResolveSourceCandidateDownloadLatestDecisionOrder(t *testing.T) {
	base := &sourceDownloadTestStore{events: []LedgerEvent{candidateDecision("source.candidate.rejected", "evt_reject", 1, "https://example.com/doc"), candidateDownloadEvent("evt_staged", 2, "https://example.com/doc", "art_candidate", "evt_proposed")}, artifacts: map[string]RawArtifact{"art_candidate": {ArtifactID: "art_candidate", MissionID: "mis_1", Content: []byte("ok")}}}
...
func TestResolveSourceSnapshotDownloadEligibility(t *testing.T) {
	...
	fresh := func() *sourceDownloadTestStore {
		return &sourceDownloadTestStore{
			snapshot: SourceSnapshot{SnapshotID: "src_1", MissionID: "mis_1", ArtifactIDs: []string{"art_pdf"}, Access: SourceAccess{RetrievalPolicy: SourceRetrievalPolicySnapshotOnly}},
			artifacts: map[string]RawArtifact{
	...
	cases := []struct {
		...
		{name: "malformed snapshot", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.snapshot = SourceSnapshot{SnapshotID: "src_1", MissionID: "mis_1", ArtifactIDs: []string{"art_pdf"}, Access: SourceAccess{RetrievalPolicy: SourceRetrievalPolicySnapshotOnly}}
		...
		{name: "removed", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.events = []LedgerEvent{{EventID: "evt_removed", Sequence: 10, EventType: SourceRemovedEvent, Payload: json.RawMessage(`{"snapshot_id":"src_1"}`)}}
			s.snapshot.State = SourceState{State: SourceStateRemoved, Removed: true}
		}},
		{name: "superseded", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.events = []LedgerEvent{{EventID: "evt_updated", Sequence: 10, EventType: ConfluenceUpdatedEvent, Payload: json.RawMessage(`{"old_snapshot_id":"src_1","new_snapshot_id":"src_2"}`)}}
			s.snapshot.State = SourceState{State: SourceStateActive, Superseded: true}
		}},
		{name: "live reference", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.snapshot.Access.RetrievalPolicy = SourceRetrievalPolicyLiveReference
		}},
		{name: "Confluence connector snapshot", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.snapshot.Connector = ConnectorRef{ConnectorID: ConfluenceConnectorID, ConnectorType: ConfluenceConnectorType}
		}},
		{name: "Liquid2 connector snapshot", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.snapshot.Connector = ConnectorRef{ConnectorID: Liquid2ConnectorID, ConnectorType: Liquid2ConnectorType}
		}},
		{name: "browser rendered HTML remains downloadable", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.snapshot.Connector = ConnectorRef{ConnectorID: "url", ConnectorType: "url", ExternalURI: "https://example.com/app"}
			s.snapshot.Locators = json.RawMessage(`[{"locator_type":"full_document","retrieval_method":"browser_render"}]`)
			s.artifacts["art_pdf"] = RawArtifact{ArtifactID: "art_pdf", MissionID: "mis_1", MediaType: "text/html", Content: []byte("<html>rendered</html>")}
		...
		{name: "cross mission artifact", request: SourceSnapshotDownloadRequest{MissionID: "mis_1", SnapshotID: "src_1"}, mutate: func(s *sourceDownloadTestStore) {
			s.artifacts["art_pdf"] = RawArtifact{ArtifactID: "art_pdf", MissionID: "mis_2"}
		}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestResolveSourceCandidateDownloadHistoricalSnapshotConsumption(t *testing.T) {
	...
	newStore := func(events []LedgerEvent) *sourceDownloadTestStore {
		return &sourceDownloadTestStore{events: append([]LedgerEvent{candidate}, events...), artifacts: map[string]RawArtifact{"art_candidate": {ArtifactID: "art_candidate", MissionID: "mis_1", Content: []byte("exact")}}}
	}
	for _, tc := range []struct {
		...
		{name: "removed historical approval", events: []LedgerEvent{
		...
		{name: "superseded historical approval", events: []LedgerEvent{
			{EventID: "evt_source", Sequence: 4, EventType: "source.snapshotted", Payload: json.RawMessage(`{"snapshot_id":"src_1","source_candidate_proposal_event_id":"evt_proposed","url":"https://example.com/doc"}`)},
		...
		{name: "browser-render historical approval", events: []LedgerEvent{
		...
		{name: "malformed historical event", events: []LedgerEvent{{EventID: "evt_bad", Sequence: 4, EventType: "source.snapshotted", Payload: json.RawMessage(`{"source_candidate_proposal_event_id":123}`)}}, wantOK: true},
		{name: "unrelated approved source", events: []LedgerEvent{{EventID: "evt_other", Sequence: 4, EventType: "source.snapshotted", Payload: json.RawMessage(`{"source_candidate_proposal_event_id":"evt_other_proposed","url":"https://example.com/other"}`)}}, wantOK: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestResolveSourceCandidateDownloadHistoricalSnapshotURLFallback(t *testing.T) {
	store := &sourceDownloadTestStore{
		...
		artifacts: map[string]RawArtifact{"art_candidate": {ArtifactID: "art_candidate", MissionID: "mis_1", Content: []byte("x")}},
	}
...
func candidateDownloadEvent(id string, sequence int64, rawURL, artifactID, proposalID string) LedgerEvent {
	payload, _ := json.Marshal(map[string]any{"url": rawURL, "artifact_id": artifactID, "proposal_event_id": proposalID})
	return LedgerEvent{EventID: id, Sequence: sequence, EventType: "source.candidate.staged", Payload: payload}
}
func candidateTerminal(kind, id string, sequence int64, rawURL string) LedgerEvent {
	payload, _ := json.Marshal(map[string]string{"url": rawURL})
	return LedgerEvent{EventID: id, Sequence: sequence, EventType: kind, Payload: payload}
}
func candidateDecision(kind, id string, sequence int64, rawURL string) LedgerEvent {
...
func cloneSourceDownloadStore(s *sourceDownloadTestStore) *sourceDownloadTestStore {
	c := *s
	c.events = append([]LedgerEvent(nil), s.events...)
	c.artifacts = map[string]RawArtifact{}
	for k, v := range s.artifacts {
		c.artifacts[k] = v
	}
	c.snapshots = append([]SourceSnapshot(nil), s.snapshots...)
	return &c
}

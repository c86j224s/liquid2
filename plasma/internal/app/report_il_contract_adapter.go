...
func (s *Service) CreateReportILBundleIfOpenContract(ctx context.Context, req reportilcontract.BundleRequest) (reportilcontract.BundleResult, error) {
	artifacts := make([]artifactcontract.CreateRequest, 0, len(req.Artifacts))
	for _, item := range req.Artifacts {
		artifacts = append(artifacts, artifactcontract.CreateRequest{ArtifactID: item.ArtifactID, MissionID: item.MissionID, MediaType: item.MediaType, Filename: item.Filename, Producer: item.Producer, Content: item.Content, ExpectedSHA256: item.ExpectedSHA256})
	}
	store := ledger.AppendRequest{EventID: req.StoreCompleted.EventID, MissionID: req.StoreCompleted.MissionID, EventType: req.StoreCompleted.EventType, CausationEventID: req.StoreCompleted.CausationEventID, CorrelationID: req.StoreCompleted.CorrelationID, Producer: req.StoreCompleted.Producer, Payload: req.StoreCompleted.Payload}
	terminal := ledger.AppendRequest{EventID: req.Terminal.EventID, MissionID: req.Terminal.MissionID, EventType: req.Terminal.EventType, CausationEventID: req.Terminal.CausationEventID, CorrelationID: req.Terminal.CorrelationID, Producer: req.Terminal.Producer, Payload: req.Terminal.Payload}

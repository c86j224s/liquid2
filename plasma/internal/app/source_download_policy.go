...
func connectorSnapshotDownloadExcluded(connector ConnectorRef) bool {
	...
	return connectorID == ConfluenceConnectorID || connectorType == ConfluenceConnectorType ||
		connectorID == Liquid2ConnectorID || connectorType == Liquid2ConnectorType
}

func sourceCandidateDownloadExcluded(normalizedURL string) bool {
...
func latestCandidateStagedEvent(events []LedgerEvent, eventID string) (LedgerEvent, sourcecandidateevents.StagedPayload, bool) {
	...
	return LedgerEvent{}, sourcecandidateevents.StagedPayload{}, false
}

func candidateLatestDecisionRejected(events []LedgerEvent, normalizedURL string) bool {
	...
	for _, event := range events {
		...
		candidateURL, err := normalizeDownloadCandidateURL(payload.URL)
...
func candidateConsumed(events []LedgerEvent, snapshots []SourceSnapshot, artifactID, normalizedURL string) bool {
...
func snapshotIdentityConsumesCandidate(snapshot SourceSnapshot, normalizedURL string) bool {
	for _, value := range []string{snapshot.Connector.ExternalURI, snapshot.Connector.ExternalSourceID} {
		candidateURL, err := normalizeDownloadCandidateURL(value)
		if err == nil && candidateURL == normalizedURL {
			return true
		}
	}
...
func legacyConfluenceExternalSourceKey(connectorID, connectorType, externalSourceID string) (string, bool) {
	...
	if connectorID != ConfluenceConnectorID && connectorType != ConfluenceConnectorType {
		return "", false
	}
...
func sourceDownloadEvents(events []LedgerEvent) []sourcecandidateevents.Event {
...
func sourceDownloadEvent(event LedgerEvent) sourcecandidateevents.Event {
...
func normalizeDownloadCandidateURL(raw string) (string, error) {

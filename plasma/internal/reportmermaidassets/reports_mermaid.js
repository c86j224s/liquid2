(function (global) {
  ...
  async function renderMermaidBlock(code, mermaid) {
    if (!code || !code.isConnected || code.dataset.plasmaMermaidState === "rendered") return true;
    ...
    try {
      ...
      global.Plasma.reports.applyPlasmaMermaidLineLegend?.(figure, source);
      global.Plasma.reports.enhanceImages?.(output);
      code.dataset.plasmaMermaidState = "rendered";
      return true;
    } catch (_error) {
  ...
  function renderPlasmaMermaid(root) {
    if (!root || !root.querySelectorAll || !global.document) return Promise.resolve(true);
    ...
    if (!blocks.length) return Promise.resolve(true);
    blocks.forEach((code) => {
    ...
    return loadMermaidRuntime()
      .then((mermaid) => {
        ...
        return Promise.all(blocks.map((code) => renderMermaidBlock(code, mermaid)));
      })
  ...
  Object.assign(global.Plasma.reports, { renderPlasmaMermaid, plasmaMermaidSanitizeOptions: mermaidSanitizeOptions });
})(window);

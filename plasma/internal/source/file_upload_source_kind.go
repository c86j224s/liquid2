...
func ClassifyUploadedFile(filename string, content []byte) (string, string, error) {
	...
	return "", "", fmt.Errorf("%w: unsupported uploaded file media type %q", producterror.ErrInvalidInput, detectedMediaType)
}

func ValidateUploadedPDF(content []byte) error {
	if !pdfdocument.IsPDFBytes(content) {
		return fmt.Errorf("%w: uploaded PDF does not contain valid PDF bytes", producterror.ErrInvalidInput)
	}
	if _, err := pdfdocument.Inspect(content); err != nil {
		return fmt.Errorf("%w: uploaded PDF inspection failed: %v", producterror.ErrInvalidInput, err)
	}
	return nil
}
...
func ValidateUploadedImage(content []byte) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w: uploaded image could not be decoded", producterror.ErrInvalidInput)
	}
	switch format {
	case "png":
	...
	default:
		return "", fmt.Errorf("%w: unsupported uploaded image format %q", producterror.ErrInvalidInput, format)
	}
}

func isUploadedArtifactText(artifact artifactcontract.Raw) bool {
...
func isUploadedArtifactMetadataOnly(artifact artifactcontract.Raw) bool {
	mediaType, _, _ := mime.ParseMediaType(artifact.MediaType)
	mediaType = strings.ToLower(strings.TrimSpace(mediaType))

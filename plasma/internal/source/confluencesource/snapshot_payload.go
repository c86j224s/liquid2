...
func BuildSnapshotPayload(
	...
	if strings.TrimSpace(page.BodyStorage) == "" || strings.TrimSpace(page.PlainText) == "" {
		return nil, nil, fmt.Errorf("%w: confluence storage body and plain text are required", producterror.ErrInvalidInput)
	}
	partial := RangeSelected(selection)
...
func RangeBody(plainText string, selection ConfluenceRangeSelection) (confluenceSnapshotBody, error) {
...
func RangeSelected(selection ConfluenceRangeSelection) bool {

...
type reportILDocumentWorkspace struct {
	WorkspaceID      string
	MissionID        string
	SessionID        string
	Stage            string
...
func (server *Server) callReportILDocumentStart(ctx context.Context, call ToolCall) ToolResult {
	...
	if strings.TrimSpace(input.Title) == "" || !utf8.ValidString(input.Title) {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL document title is invalid", false, nil)
	}
	if !reportILDocumentLanguageValid(input.Language) {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL document language is invalid", false, nil)
	}

	directSourceAuthor := binding.MaxReadBytes > 0
	server.mu.Lock()
	memoryRead := server.reportILEditorialMemoryReviewComplete
	sourceRead := server.reportILCompleteCatalogRead(binding)
	server.mu.Unlock()
	if directSourceAuthor {
		if !sourceRead {
			return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL document workspace requires the complete selected source read", false, nil)
		}
	} else if !memoryRead {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL document workspace requires the complete editorial memory read", false, []string{binding.EditorialMemoryArtifactID})
	}
	...
	if len(server.reportILDocumentWorkspaces) != 0 {
		return errorResult(call.Name, binding.Catalog.MissionID, "conflict", "report IL document workspace already exists", false, nil)
	}
	now := time.Now().UTC()
	workspace := &reportILDocumentWorkspace{
		WorkspaceID:     newMCPID("ilw"),
		MissionID:       binding.Catalog.MissionID,
		SessionID:       server.binding.AgentSessionID,
		Stage:           binding.Stage,
	...
	server.reportILDocumentWorkspaces[workspace.WorkspaceID] = workspace
	return ToolResult{ToolName: call.Name, MissionID: workspace.MissionID, Content: reportILDocumentState(*workspace)}
}
...
func (server *Server) callReportILDocumentOpen(ctx context.Context, call ToolCall) ToolResult {
	...
	artifact, err := server.service.GetRawArtifact(ctx, binding.BaseAuthorArtifactID)
	if err != nil {
		return errorFromErr(call.Name, binding.Catalog.MissionID, err, []string{binding.BaseAuthorArtifactID})
	}
	if artifact.MissionID != binding.Catalog.MissionID ||
		artifact.MediaType != reportilcontract.AuthorDocumentMediaType ||
		artifact.SHA256 != binding.BaseAuthorSHA256 || sha256Hex(artifact.Content) != binding.BaseAuthorSHA256 ||
		int64(len(artifact.Content)) != artifact.ByteSize {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL publication base document binding is invalid", false, []string{binding.BaseAuthorArtifactID})
	}
	decoder := json.NewDecoder(bytes.NewReader(artifact.Content))
	decoder.DisallowUnknownFields()
	var document reportilcontract.AuthorDocument
	if err := decoder.Decode(&document); err != nil {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL publication base document is invalid", false, []string{binding.BaseAuthorArtifactID})
	}
	var extra any
	...
	if validationErr := reportilcontract.ValidateAuthorDocumentWithMemory(document, memory, binding.Catalog); validationErr != nil {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", validationErr.Error(), false, []string{binding.BaseAuthorArtifactID, binding.EditorialMemoryArtifactID})
	}

	now := time.Now().UTC()
	workspace := &reportILDocumentWorkspace{
		WorkspaceID: newMCPID("ilw"), MissionID: binding.Catalog.MissionID,
		SessionID: server.binding.AgentSessionID, Stage: binding.Stage, CatalogSHA256: binding.Catalog.SHA256,
...
func (server *Server) callReportILDocumentAppend(ctx context.Context, call ToolCall) ToolResult {
	...
	server.mu.Lock()
	defer server.mu.Unlock()
	workspace, err := server.reportILDocumentWorkspace(input.WorkspaceID, binding)
	if err != nil {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", err.Error(), false, nil)
	}
	if workspace.Finalized || workspace.Finalizing {
		return errorResult(call.Name, workspace.MissionID, "conflict", "report IL document is already finalizing or finalized", false, []string{workspace.WorkspaceID, workspace.ArtifactID})
	}
...
func (server *Server) callReportILDocumentAppendSource(ctx context.Context, call ToolCall) ToolResult {
	...
	if binding.MaxReadBytes == 0 {
		return errorResult(call.Name, binding.Catalog.MissionID, "binding", "report IL direct source authoring is unavailable for this stage", false, nil)
	}
...
func (server *Server) callReportILDocumentRead(ctx context.Context, call ToolCall) ToolResult {
	...
	if input.Offset < 0 || input.MaxBytes < 1 || input.MaxBytes > reportILDocumentMaxReadBytes {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL document read range is invalid", false, []string{input.WorkspaceID})
	}
	server.mu.Lock()
	defer server.mu.Unlock()
	workspace, err := server.reportILDocumentWorkspace(input.WorkspaceID, binding)
	...
	if workspace.ReviewRevision != workspace.Revision {
		if input.Offset != 0 {
			return errorResult(call.Name, workspace.MissionID, "validation", "report IL document reread must start at offset 0", false, []string{workspace.WorkspaceID})
		}
	...
	if workspace.ReviewComplete || input.Offset != workspace.ReviewNextOffset {
		return errorResult(call.Name, workspace.MissionID, "validation", fmt.Sprintf("report IL document reads must continue at exact next_offset %d", workspace.ReviewNextOffset), false, []string{workspace.WorkspaceID})
	}
	review := reportILDocumentReviewText(workspace.Document)
	content, offset, nextOffset, truncated, err := boundedArtifactContentWithLimit([]byte(review), input.Offset, input.MaxBytes, input.MaxBytes)
	if err != nil {
		return errorResult(call.Name, workspace.MissionID, "validation", err.Error(), false, []string{workspace.WorkspaceID})
	}
	if truncated {
	...
	return ToolResult{ToolName: call.Name, MissionID: workspace.MissionID, Content: reportILDocumentReadOutput{
...
func (server *Server) callReportILDocumentReplace(ctx context.Context, call ToolCall) ToolResult {
	...
	if strings.TrimSpace(input.OldText) == "" || input.OldText == input.NewText ||
		...
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL document replacement is invalid", false, []string{input.WorkspaceID})
	}
	server.mu.Lock()
	defer server.mu.Unlock()
	workspace, err := server.reportILDocumentWorkspace(input.WorkspaceID, binding)
	...
	if !replaceUniqueReportILDocumentText(&candidate, input.OldText, input.NewText) {
		return errorResult(call.Name, workspace.MissionID, "validation", "old_text must occur exactly once in one reader-facing document value", false, []string{workspace.WorkspaceID})
	}
...
func (server *Server) callReportILDocumentEditText(ctx context.Context, call ToolCall) ToolResult {
	...
	if strings.TrimSpace(input.TargetKey) == "" || strings.TrimSpace(input.OldText) == "" ||
		...
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL publication text edit is invalid", false, []string{input.WorkspaceID})
	}
	server.mu.Lock()
	defer server.mu.Unlock()
	workspace, err := server.reportILDocumentWorkspace(input.WorkspaceID, binding)
	...
	if !editUniqueReportILDocumentReaderText(&candidate, input.TargetKind, strings.TrimSpace(input.TargetKey), input.OldText, input.NewText) {
		return errorResult(call.Name, workspace.MissionID, "validation", "target_kind and target_key must identify one editable reader-facing value where old_text occurs exactly once", false, []string{workspace.WorkspaceID})
	}
	if err := reportilcontract.ValidateAuthorDocumentPartial(candidate, binding.Catalog); err != nil {
		return errorResult(call.Name, workspace.MissionID, "validation", err.Error(), false, []string{workspace.WorkspaceID})
	}
...
func (server *Server) callReportILDocumentReviseBlock(ctx context.Context, call ToolCall) ToolResult {
	...
	if strings.TrimSpace(input.BlockKey) == "" || strings.TrimSpace(input.OldText) == "" ||
		...
		len([]byte(input.OldText)) > reportILDocumentMaxReplaceBytes ||
		len([]byte(input.NewText)) > reportILDocumentMaxReplaceBytes {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "report IL document block revision is invalid", false, []string{input.WorkspaceID})
	}
	server.mu.Lock()
	defer server.mu.Unlock()
	workspace, err := server.reportILDocumentWorkspace(input.WorkspaceID, binding)
	...
	if !reviseReportILDocumentBlock(&candidate, strings.TrimSpace(input.BlockKey), input.OldText, input.NewText, input.EditorialAccountKeys, sourceKeys) {
		return errorResult(call.Name, workspace.MissionID, "validation", "block_key must identify one content block where old_text occurs exactly once", false, []string{workspace.WorkspaceID})
	}
	if err := reportilcontract.ValidateAuthorDocumentPartial(candidate, binding.Catalog); err != nil {
		return errorResult(call.Name, workspace.MissionID, "validation", err.Error(), false, []string{workspace.WorkspaceID})
	}
...
func (server *Server) callReportILDocumentFinalize(ctx context.Context, call ToolCall) ToolResult {
	...
	if !workspace.ReviewComplete || workspace.ReviewRevision != workspace.Revision {
		server.mu.Unlock()
		return errorResult(call.Name, workspace.MissionID, "validation", "report IL document must be reread completely after its last edit", false, []string{workspace.WorkspaceID})
	}
	...
	if validationErr != nil {
		server.mu.Unlock()
		return errorResult(call.Name, workspace.MissionID, "validation", validationErr.Error(), false, []string{workspace.WorkspaceID})
	}
	content, err := json.Marshal(workspace.Document)
	if err != nil {
		server.mu.Unlock()
		return errorResult(call.Name, workspace.MissionID, "internal", "report IL document encoding failed", false, []string{workspace.WorkspaceID})
	}
	...
	if (workspaceCopy.Stage == "il_continuity" || workspaceCopy.Stage == "il_reader") && workspaceCopy.Replacements == 0 {
		storedArtifact, err = server.service.GetRawArtifact(ctx, workspaceCopy.BaseArtifactID)
		...
		storedArtifact, err = server.service.CreateRawArtifact(ctx, artifact.CreateRequest{
			...
			Producer:  ledger.Producer{Type: "mcp_tool", ID: ToolReportILDocumentFinalize},
			Content:   content,
		})
	}
	if err != nil {
		server.mu.Lock()
		if current, currentErr := server.reportILDocumentWorkspace(input.WorkspaceID, binding); currentErr == nil && current.Revision == workspaceCopy.Revision {
	...
	if err != nil || workspace.Finalized || !workspace.Finalizing || workspace.Revision != workspaceCopy.Revision {
		return errorResult(call.Name, workspaceCopy.MissionID, "conflict", "report IL document changed during finalization", false, []string{workspaceCopy.WorkspaceID, storedArtifact.ArtifactID})
	}
...
func (server *Server) reportILAuthorDocumentBinding() (reportilcontract.SourceAccessBinding, error) {
...
func (server *Server) reportILPublicationDocumentBinding() (reportilcontract.SourceAccessBinding, error) {
	binding, err := server.reportILSourceAccessBinding()
	if err != nil {
		return reportilcontract.SourceAccessBinding{}, err
	}
...
func (server *Server) reportILContinuityDocumentBinding() (reportilcontract.SourceAccessBinding, error) {
	binding, err := server.reportILSourceAccessBinding()
	if err != nil {
		return reportilcontract.SourceAccessBinding{}, err
	}
...
func (server *Server) reportILPublicationReaderDocumentBinding() (reportilcontract.SourceAccessBinding, error) {
	binding, err := server.reportILSourceAccessBinding()
	if err != nil {
		return reportilcontract.SourceAccessBinding{}, err
	}
...
func (server *Server) reportILDocumentBinding() (reportilcontract.SourceAccessBinding, error) {
	binding, err := server.reportILSourceAccessBinding()
	if err != nil {
		return reportilcontract.SourceAccessBinding{}, err
	}
...
func (server *Server) reportILCompleteCatalogRead(binding reportilcontract.SourceAccessBinding) bool {
...
func (server *Server) reportILDocumentWorkspace(workspaceID string, binding reportilcontract.SourceAccessBinding) (*reportILDocumentWorkspace, error) {
	workspace := server.reportILDocumentWorkspaces[strings.TrimSpace(workspaceID)]
...
func (server *Server) reportILDocumentMutated(workspace *reportILDocumentWorkspace) {
	workspace.Revision++
...
func reportILDocumentState(workspace reportILDocumentWorkspace) reportILDocumentStateOutput {
...
func (server *Server) appendReportILDocumentBlock(
	workspace *reportILDocumentWorkspace,
	...
	if reportILDocumentBlockCount(workspace.Document) > reportilcontract.MaxAuthorBlocks {
		section.Blocks = section.Blocks[:len(section.Blocks)-1]
		return fmt.Errorf("report IL document block inventory exceeds the ceiling")
	}
...
func (server *Server) reportILBoundEditorialMemory(
	ctx context.Context,
	binding reportilcontract.SourceAccessBinding,
	...
	if artifactValue.MissionID != binding.Catalog.MissionID ||
		...
		sha256Hex(artifactValue.Content) != binding.EditorialMemorySHA256 ||

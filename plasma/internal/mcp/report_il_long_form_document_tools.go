...
func (server *Server) callReportILLongFormDocumentStart(ctx context.Context, call ToolCall) ToolResult {
	...
	server.mu.Lock()
	planRead := server.reportILLongFormPlanReviewComplete
	server.mu.Unlock()
	if !planRead {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form document requires the complete bound plan read", false, []string{binding.LongFormPlanArtifactID})
	}
	var input reportILLongFormDocumentStartInput
	if err := decodeArgs(call.Arguments, &input); err != nil || strings.TrimSpace(input.Title) == "" ||
		!utf8.ValidString(input.Title) || !reportILDocumentLanguageValid(input.Language) {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form document start input is invalid", false, nil)
	}
	...
	if binding.Stage == "il_long_form_section" && binding.MaxReadBytes > 0 {
		if !server.reportILBoundSourcesComplete(binding.LongFormSourceKeys) {
			return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form Section requires the complete bound source read", false, nil)
		}
		...
		server.mu.Unlock()
		if !memoryRead {
			return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form document requires the complete editorial memory read", false, []string{binding.EditorialMemoryArtifactID})
		}
		...
		return errorResult(call.Name, binding.Catalog.MissionID, "binding", "long-form Section source binding is unavailable", false, nil)
	}

	document, inputs, err := server.reportILLongFormDocumentSeed(ctx, binding, input)
	if err != nil {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", err.Error(), false, inputs)
	}
	...
	if len(server.reportILDocumentWorkspaces) != 0 {
		return errorResult(call.Name, binding.Catalog.MissionID, "conflict", "long-form document workspace already exists", false, inputs)
	}
...
func (server *Server) reportILLongFormDocumentSeed(
	ctx context.Context,
	...
	for index, artifactID := range binding.LongFormInputArtifactIDs {
		artifactValue, err := server.service.GetRawArtifact(ctx, artifactID)
		if err != nil {
		...
		if artifactValue.MissionID != binding.Catalog.MissionID ||
			...
			sha256Hex(artifactValue.Content) != binding.LongFormInputSHA256s[index] ||
...
func (server *Server) callReportILLongFormDocumentAppend(ctx context.Context, call ToolCall) ToolResult {
	...
	if err != nil || workspace.Finalized || workspace.Finalizing {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form Section workspace is unavailable", false, []string{input.WorkspaceID})
	}
	if len(workspace.Document.Parts) != 1 || len(workspace.Document.Parts[0].Sections) != 1 {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form Section workspace inventory is invalid", false, []string{input.WorkspaceID})
	}
	...
	if input.SectionKey != "" && input.SectionKey != section.SectionKey ||
		...
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form Section append target is invalid", false, []string{input.WorkspaceID})
	}
	...
	if err := validateReportILLongFormBlockBinding(workspace, binding, block); err != nil {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", err.Error(), false, []string{input.WorkspaceID})
	}
...
func (server *Server) callReportILLongFormDocumentCorrectBlock(ctx context.Context, call ToolCall) ToolResult {
	...
	if err := decodeArgs(call.Arguments, &input); err != nil || strings.TrimSpace(input.BlockKey) == "" ||
		...
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form Section block correction is invalid", false, []string{input.WorkspaceID})
	}
	server.mu.Lock()
	defer server.mu.Unlock()
	workspace, err := server.reportILDocumentWorkspace(input.WorkspaceID, binding)
	...
	if !workspace.ReviewComplete || workspace.ReviewRevision != workspace.Revision {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form Section must be read completely before correcting a block", false, []string{input.WorkspaceID})
	}
	...
	for index := range section.Blocks {
		if section.Blocks[index].BlockKey == input.BlockKey {
			if blockIndex != -1 {
				return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form Section block identity is invalid", false, []string{input.WorkspaceID})
			}
			blockIndex = index
		}
	}
	if blockIndex == -1 {
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "block_key must identify one draft block in the bound Section", false, []string{input.WorkspaceID})
	}
...
func (server *Server) callReportILLongFormDocumentFinalize(ctx context.Context, call ToolCall) ToolResult {
	...
	if err != nil || workspace.Finalized || workspace.Finalizing || !workspace.ReviewComplete || workspace.ReviewRevision != workspace.Revision {
		server.mu.Unlock()
		return errorResult(call.Name, binding.Catalog.MissionID, "validation", "long-form document must be read completely after its final edit", false, []string{input.WorkspaceID})
	}
	...
	if err != nil {
		server.mu.Unlock()
		return errorResult(call.Name, binding.Catalog.MissionID, "internal", "long-form document encoding failed", false, []string{input.WorkspaceID})
	}
	...
	stored, err := server.service.CreateRawArtifact(ctx, artifact.CreateRequest{
		...
		Producer: ledger.Producer{Type: "mcp_tool", ID: ToolReportILLongFormDocumentFinalize}, Content: content,
	})
	if err != nil {
		server.mu.Lock()
	...
	if err != nil || workspace.Finalized || !workspace.Finalizing || workspace.Revision != workspaceCopy.Revision {
		return errorResult(call.Name, workspaceCopy.MissionID, "conflict", "long-form document changed during finalization", false, []string{workspaceCopy.WorkspaceID, stored.ArtifactID})
	}
...
func (server *Server) validateReportILLongFormWorkspace(workspace *reportILDocumentWorkspace, binding reportilcontract.SourceAccessBinding) error {
...
func (server *Server) reportILLongFormDocumentBinding() (reportilcontract.SourceAccessBinding, error) {
...
func (server *Server) reportILLongFormSectionDocumentBinding() (reportilcontract.SourceAccessBinding, error) {
	binding, err := server.reportILLongFormDocumentBinding()
	if err != nil {
		return reportilcontract.SourceAccessBinding{}, err
	}
...
func (server *Server) reportILValidateLongFormInputStage(
	ctx context.Context,
	binding reportilcontract.SourceAccessBinding,
	artifactID string,
) error {
	events, err := server.service.ListEvents(ctx, binding.Catalog.MissionID)
	if err != nil {
		return err
	}
...
func (server *Server) reportILBoundLongFormPlanSection(ctx context.Context, binding reportilcontract.SourceAccessBinding) (reportilcontract.LongFormPart, reportilcontract.LongFormSection, error) {
	plan, err := server.reportILBoundLongFormPlan(ctx, binding)
	if err != nil {
...
func (server *Server) reportILBoundLongFormPlan(ctx context.Context, binding reportilcontract.SourceAccessBinding) (reportilcontract.LongFormPlan, error) {

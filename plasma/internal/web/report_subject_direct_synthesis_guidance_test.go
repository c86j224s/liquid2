...
func TestSubjectDirectSynthesisProfileTargetsPlanningAndSectionOnly(t *testing.T) {
	profile, sha, err := SelectReportGenerationGuidanceForMode(reportModeLongForm, reportGenerationGuidanceProfilePartConnectiveSubjectDirectSynthesisVoice)
	if err != nil || profile != reportGenerationGuidanceProfilePartConnectiveSubjectDirectSynthesisVoice || strings.TrimSpace(sha) == "" {
	...
	for _, mode := range []string{reportModeOneTake, reportModePlanned} {
		if _, _, err := SelectReportGenerationGuidanceForMode(mode, profile); err == nil {
	...
	if !strings.Contains(reportSectionDirectWritingGuidance(profile), "Section direct-writing guidance:") {
		t.Fatal("subject-direct profile did not inherit Section direct-writing guidance")
	}
	if !strings.Contains(reportPartConnectiveEconomyGuidance(profile), "Part connective-economy guidance:") {
		t.Fatal("subject-direct profile did not inherit Part connective-economy guidance")
	}
	if strings.Contains(LongFormReportGenerationGuidance(profile), "Long-form section-brief cluster-memory guidance:") {
	...
	defaultPartPrompt := agentPartAssemblyPrompt("Long", "mis_1", "ses_1", reportRigorProfiles["balanced"], plan, part, drafts, 0, reportGenerationGuidanceProfilePartConnectiveEconomyVoice)
	...
	defaultFinalPrompt := agentLongFormFinalizePrompt("Long", binding.MissionID, reportRigorProfiles["balanced"], plan, nil, reportGenerationGuidanceProfilePartConnectiveEconomyVoice, binding, 1, false, reporting.LongFormFinalizationHint{})

...
func reportRequirementStageMatches(pendingEventID, planEventID string, event ledger.Event) bool {
	var payload struct {
...
func applyReportRequirementMapProgress(pendingEventID, planEventID string, event ledger.Event, progress *sectionalReportProgress) error {
	var payload struct {
		PendingEventID string                         `json:"pending_event_id"`
		PlanEventID    string                         `json:"plan_event_id"`

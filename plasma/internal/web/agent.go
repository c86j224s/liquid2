...
func researchToolOrientation(resumed bool) string {
	if resumed {
		return "Continue from the existing session context and do not re-read plasma.research.outline or plasma.research.list only to regain orientation. When mission changes need checking, call plasma.research.changes with the last confirmed sequence from an earlier outline or changes response, then retain current_sequence. If no confirmed sequence is available or resync_required is true, read plasma.research.outline instead."
	}
	return "Start with plasma.research.outline and retain its last_sequence for later change checks."
}

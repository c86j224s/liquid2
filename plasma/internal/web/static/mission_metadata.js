(function missionMetadataModule(Plasma) {
  "use strict";
  ...
  function initMissionMetadataEditor() {
    const form = $("missionMetadataForm");
    if (!form) return;
    ...
    form.addEventListener("submit", saveMissionMetadata);
    renderMissionMetadataEditor(null, true);
  }

  async function saveMissionMetadata(event) {
    event.preventDefault();
    if (!state.missionId) return;
    const owner = mission.captureMissionSelection();
    $("missionMetadataError").textContent = "";
    try {
      await missionApi(owner, "", {
        method: "PATCH",
        body: {
      ...
      $("missionMetadataForm").dataset.dirty = "false";
      await mission.loadMissions();
      await mission.reloadMission();
    } catch (err) {
      if (!mission.isStaleMissionOperation(err) && mission.ownsMissionSelection(owner)) $("missionMetadataError").textContent = err.userMessage || err.message;
    }
  }

  Object.assign(mission, {
    initMissionMetadataEditor,
    missionMetadataFieldIDs,
    missionMetadataLines,
    renderMissionMetadataEditor,
    setMissionMetadataDisabled
  });
})(window.Plasma);

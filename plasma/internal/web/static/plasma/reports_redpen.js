(function (global) {
  ...
  function createReportRedpenController(options) {
    ...
    let view = null, active = false, busy = false, inlineEditor = null, selectedBlock = null;
    let draft = "", saved = "", currentArtifactID = "", revision = 0;
    ...
    function captureSelection() {
      ...
      setSelectedBlock(global.Plasma.reports.redpenMarkdown.selectionBlock(body, selection));
      selection.removeAllRanges();
    }
    function selectBlockOnClick(event) {
      if (!active || inlineEditor) return;
      const block = global.Plasma.reports.redpenMarkdown.blockForTarget(body, event.target);
      if (!block) return;
      event.preventDefault();
    ...
    function editSelectedBlock() {
      ...
      const raw = global.Plasma.reports.redpenMarkdown.rawBlock(draft, startLine, endLine);
      inlineEditor = global.Plasma.reports.redpenMarkdown.createInlineEditor(element, raw, {
        apply(value) {
          draft = global.Plasma.reports.redpenMarkdown.replaceBlock(draft, startLine, endLine, value);
          inlineEditor = null;
    ...
    function updateToolbar() {
      ...
      status.textContent = busy ? "처리 중" : inlineEditor ? "블록 편집 중" : hasUnsavedChanges() ? "저장 안 됨" : revision ? `v${revision} 저장됨` : "빨간펜 켜짐";
    }
    function reset() {
      view = null;
      active = false;
  ...
  global.Plasma.reports.createRedpenController = createReportRedpenController;
})(window);

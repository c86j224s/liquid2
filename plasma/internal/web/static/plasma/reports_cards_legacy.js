(function reportsLegacyCards(root) {
  ...
  function renderLegacyReportCard(key, report, selectedKey, isLatest = report.isLatest) {
    return `<div class="item report-card ${isLatest ? "active" : ""} ${key === selectedKey ? "selected" : ""}" data-report-key="${escapeAttr(key)}" data-report-card-version-id="${escapeAttr(report.versionID)}">
      <div class="item-title report-title-line report-card-toggle"><span>${escapeHTML(report.title)}</span><span class="chip-row report-chip-row">${isLatest ? `<span class="badge session-new">최신</span>` : `<span class="badge muted">이전</span>`}${report.rigorLabel ? `<span class="badge">${escapeHTML(report.rigorLabel)}</span>` : ""}<span class="badge muted">${escapeHTML(report.stateLabel)}</span></span></div>
  ...
  reports.renderLegacyReportCard = renderLegacyReportCard;
})(window);

...
func TestCompleteReportRunRejectsActualLineageMismatch(t *testing.T) {
	s := completionTestStore{events: baseCompletionEvents()}
	_, err := CompleteReportRun(context.Background(), &s, ReportCompletionRequest{MissionID: "mis_1", CanonicalEventID: "evt_art", ActualUsage: &reportusage.ReportAgentUsageRequest{MissionID: "mis_1", CanonicalEventID: "evt_target", AgentSessionID: "ses-other", Surface: "report_wrong", Usage: agentusage.New("", "", "", "", "").WithUnavailable("lost")}})
	if err == nil || len(s.events) != 3 {

...
func TestStepPromptUsesLayeredShapeForLegacyCurrentMode(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"useful remaining work that you can start now",
		"immediately executable next_instruction",
		"useful synthesis, comparison, organization, or concise user-visible summary",
		"no useful investigation or organization step remains executable now",
		"Do not use stop merely because the current step instruction is complete",
		"Waiting for user approval or a reply, a source attachment, credentials, or a future external event is not executable workflow work",
		"A pending source candidate, its staging, or its user review does not by itself keep the workflow open",
		"Explain the current result and the condition for resuming",
		"Do not use \"continue\" to re-check the same unchanged condition",
		"소스 후보:",
		"채택 의견:",

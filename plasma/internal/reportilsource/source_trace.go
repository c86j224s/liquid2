...
// VerifySourceRead confirms the exact stage and forward-only content
...
func VerifySourceRead(ctx context.Context, listEvents func(context.Context, string) ([]ledger.Event, error), missionID, toolSessionID, expectedStage string, catalog reportilcontract.SourceCatalog) (reportilcontract.SourceReadReceipt, error) {
	...
	events, err := listEvents(ctx, missionID)
	if err != nil {
		return reportilcontract.SourceReadReceipt{}, err
	}

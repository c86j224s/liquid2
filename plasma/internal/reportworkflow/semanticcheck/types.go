...
const Stage = reporting.FinalEditStageStyleSemanticValidation

// Runner는 semantic validation stage의 prompt, tools, durable replay를 실행한다.
type Runner struct {
	Store reporting.FinalEditStageStore
	NewID finaledit.IDGenerator
}

// Input은 validation source와 plan-session fork lineage를 고정한다.
type Input struct {
...
// Output은 semantic validation submission 결과다.
type Output struct {
	Run finaledit.StageRun
}

// Run은 기존 semantic validation 두-attempt/replay 계약으로 stage를 실행한다.
func (runner Runner) Run(ctx context.Context, input Input, executor agentexec.AgentExecutor, forker agentexec.AgentSessionForker) (Output, error) {
	...
	progress, err := core.StageProgress(ctx, input.FinalBase, input.Base, Stage, input.SourceArtifactID, core.ID("art"), input.PlanSessionID, forker, input.PlanSessionID)
	if err != nil {
		return Output{}, err
	}

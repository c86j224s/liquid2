...
func (store *reportAgentUsageTestStore) AppendEventConditionally(_ context.Context, missionID string, decide func([]ledger.Event) (ledger.AppendRequest, ledger.Event, bool, error)) (ledger.Event, bool, error) {
	request, existing, appendEvent, err := decide(append([]ledger.Event(nil), store.events...))

...
func TestRunServeRecoversReportCompletionBeforeListening(t *testing.T) {
	...
	if _, err := svc.CreateMission(ctx, app.CreateMissionRequest{MissionID: "mis_report_recovery", Title: "Recovery"}); err != nil {
		t.Fatal(err)
	}
	for _, event := range []app.AppendEventRequest{
		{EventID: "evt_report_recovery_root", MissionID: "mis_report_recovery", EventType: "report.draft.pending", Producer: app.Producer{Type: "user", ID: "test"}, Payload: []byte(`{"retry_strategy":"initial"}`)},
		{EventID: "evt_report_recovery_target", MissionID: "mis_report_recovery", EventType: "report.requirements.mapped", Producer: app.Producer{Type: "agent_session", ID: "ses_recovery"}, Payload: []byte(`{"pending_event_id":"evt_report_recovery_root","previous_provider_session_id":"ses_recovery","agent_executor":"codex"}`)},
	} {
		if _, err := svc.AppendEvent(ctx, event); err != nil {
			t.Fatal(err)
		}
	}
	artifact, _, err := svc.CreateRawArtifactWithEvent(ctx, app.CreateRawArtifactRequest{ArtifactID: "art_report_recovery", MissionID: "mis_report_recovery", MediaType: "text/markdown", Filename: "report.md", Producer: app.Producer{Type: "agent", ID: "test"}, Content: []byte("# recovered")}, func(a app.RawArtifact) app.AppendEventRequest {
		return app.AppendEventRequest{EventID: "evt_report_recovery_final", MissionID: "mis_report_recovery", EventType: "report.artifact.created", Producer: app.Producer{Type: "agent", ID: "test"}, Payload: []byte(`{"pending_event_id":"evt_report_recovery_root","artifact_id":"` + a.ArtifactID + `"}`)}
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	listen := func(*http.Server) error {
		...
		var usageEvent, completionEvent app.LedgerEvent
		...
		if usageEvent.EventID != "evt_report_usage_report_recovery_target" || usageEvent.Producer != (app.Producer{Type: "agent_session", ID: "ses_recovery"}) || usageEvent.CausationEventID != "evt_report_recovery_target" || usageEvent.CorrelationID != "evt_report_recovery_root" {
		...
		if completionEvent.EventID != "evt_report_run_completed_report_recovery_root" || completionEvent.Producer != (app.Producer{Type: "system", ID: "report-completion"}) || completionEvent.CausationEventID != "evt_report_recovery_final" || completionEvent.CorrelationID != "evt_report_recovery_root" {
